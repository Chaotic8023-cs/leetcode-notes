...
class Solution:
    ...
    补充：本题为什么只能排序然后用prev横向去重，而不能用#491题非递减子序列那样用一个（per-level）的used集合来做横向去重？
    
        40. 组合总和 II：答案是“组合”（多重集合），不关心顺序。如果不排序，就可能从不同层次按不同顺序拿到同一组数（比如先拿 2 再拿 1，或先拿 1 再拿 2），这会生成同一组合的不同排列。
        ⇒ 仅靠“每层 used 去重”挡不住跨层的排列重复，必须先排序来强制非递减生成顺序，把“排列问题”消掉。

        491. 递增子序列：答案是子序列，必须保持原数组的索引顺序，并且要求非递减。
        ⇒ 这里天然有一个“唯一生成顺序”：索引递增 + 值非递减。你不可能先选一个更大的数再回头选更小的数（会被 nums[i] >= state[-1] 卡住），因此跨层的排列重复根本不会产生。
        真正会产生的重复来自：“同一层里有多个相同值”（比如两个 7），无论选哪个 7，后续能延展出的值序列是一模一样的。这正是per-level 的 used 要解决的：在同一层只允许某个值展开一次，就够了。
    """
